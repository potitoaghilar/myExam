...
class Game {
    ...
    static get questionAmount() { return 30; }
    static get category() { return 18; } //non ci serve dato che, non usiamo più la sorgente esterna
    ...
    constructor(difficulty) {
      //  console.log('New game started in mode ' + difficulty);
        ...
        this.time = 30 * 60;
		this.time = 10; //debug only
		this.matricola= document.getElementById("matricola").value;
		this.nome =document.getElementById("nome").value;
		this.cognome =document.getElementById("cognome").value;
    ...
    fetchQuestions() {
        ...
        $.ajax({
			...
			url: 'assets/script/questions.php',
            success: (data) => {
    ...
    draw() {
		...
		//pulsante precendente
		const prevBtnHTML = '<button type="button" id="prevQuestion" class="btn btn-primary mt-3">Precedente</button>';
        ...
        $('#game').html(questionHTML + '<div class="mx-auto mt-3 w-50 text-left">' + answersHTML + '</div>' + prevBtnHTML + nextBtnHTML);
    ...
    showResults() {
		...
		//inizio caricamento punteggio
		$.ajax({    
        url: "assets/script/publish.php?matricola="+this.matricola+"&results="+this.points+"&nome="+this.nome+"&cognome="+this.cognome,             
        dataType: "html",                   
